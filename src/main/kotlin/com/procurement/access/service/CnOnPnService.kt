...
class CnOnPnService(
    ...
    fun checkCnOnPn(cm: CommandMessage): ResponseDto {
        ...
        if (pnEntity.tender.items.isEmpty()) {
            ...
            if (request.tender.electronicAuctions != null) {
                ...
                val percentEligibleMinimumDifference: BigDecimal =
                    percentEligibleMinimumDifference(contextRequest = contextRequest)
    ...
    private fun checkContractPeriodInLotsWhenPNWithoutItemsFromRequest(
        ...
        when (contextRequest.pmd) {
            ...
            ProcurementMethod.FA, ProcurementMethod.TEST_FA ->
                throw ErrorException(INVALID_PMD)
        }
    }
     ...
     * ELSE eAuction throws Exception;
     */
    private fun percentEligibleMinimumDifference(
        contextRequest: ContextRequest
    ): BigDecimal {
        if (contextRequest.country == "MD") {
            return when (contextRequest.pmd) {
                ProcurementMethod.OT, ProcurementMethod.TEST_OT,
                ProcurementMethod.SV, ProcurementMethod.TEST_SV,
                ProcurementMethod.MV, ProcurementMethod.TEST_MV -> BigDecimal(0.1).setScale(2, RoundingMode.HALF_UP)
            ...
            throw ErrorException(error = CONTEXT, message = "Invalid a country.")
    }
    ...
    private fun checkContractPeriodInLotsFromRequestWhenPNWithItems(
        ...
        when (contextRequest.pmd) {
            ...
            ProcurementMethod.FA, ProcurementMethod.TEST_FA,
            ProcurementMethod.RT, ProcurementMethod.TEST_RT -> throw ErrorException(INVALID_PMD)
        }
    }
    ...
    private fun tender(
        ...
        val statusDetails: TenderStatusDetails = when (contextRequest.pmd) {
            ProcurementMethod.OT, ProcurementMethod.TEST_OT,
            ProcurementMethod.SV, ProcurementMethod.TEST_SV,
            ProcurementMethod.MV, ProcurementMethod.TEST_MV -> TenderStatusDetails.CLARIFICATION
    ...
    private fun convertElectronicAuctionsFromRequest(
        ...
        return when (contextRequest.pmd) {
            ...
            ProcurementMethod.SV, ProcurementMethod.TEST_SV,
            ProcurementMethod.MV, ProcurementMethod.TEST_MV -> {
                tenderFromRequest.electronicAuctions?.let {
